...
- current quantized GitLab-coverage rerun to watch: `outputs/qwen_bootstrap41_curriculum_v19_qlora_gitlabcoverage_cleandemos_warmupoversample_fix/`, which keeps the same checked-in `bootstrap41` split, `bnb_4bit` step-weight recipe, GitLab budget, GitLab coverage bump, clean-demo fix, and GitLab warmup oversampling as `v18` but now includes the staged-runner override-contract fix
...
- both order-only paths now also have saved dry-run preflights under `outputs/shopping_order_curriculum_preflight_v1/` and `outputs/shopping_order_full_curriculum_preflight_v1/`, so their checked-in manifests are already validated against the current recommended splits
