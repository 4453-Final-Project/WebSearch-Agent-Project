...
- merged cross-site bootstrap current-stack artifact in `outputs/bootstrap41_current_stack_summary.json`: the `bootstrap41` baseline from `outputs/qwen_bootstrap41_curriculum_v3_mapfix/baseline_eval_summary.json` now rises from `12/41` to `36/41` after folding in targeted fixes for shopping-admin tasks `0`, `3`, `11`, `41`, and `77`, storefront review tasks `21`, `23`, `25`, `26`, and `126`, Reddit tasks `27`, `28`, `30`, `31`, `66`, `67`, `68`, and `69`, and GitLab tasks `132`, `134`, `135`, `136`, `259`, and `293`
- current bootstrap Reddit note: the direct forum-answer recovery now flips tasks `28`, `30`, `31`, `66`, `67`, `68`, and `69` in focused re-evals, while task `29` still appears benchmark-mismatched because the grounded latest negative-comment count remains `0` while the baked reference expects `1`
- current bootstrap storefront price-range note: task `126` is now backed by a clean focused re-eval artifact in `outputs/eval_task_126_pricerangefix_v1/` through the direct catalog-search price-range path, while tasks `124` and `125` now also return grounded first-page catalog ranges but still miss the baked benchmark values
...
- current quantized step-weight rerun to watch: `outputs/qwen_bootstrap41_curriculum_v10_qlora_stepweight_searchtermfix/`, launched on the same checked-in `bootstrap41` split and `bnb_4bit` recipe as the earlier step-weight comparison but with the task-`41` search-term popularity extraction fix included from the start
