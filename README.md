...
- the next queued bootstrap comparison path is checked in too: `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_adminoversample_tmux.sh` keeps the same `v22` stack but also boosts `site_shopping_admin` warmup coverage and warmup sampling, so if `v22` still shows early admin fragility the follow-up rerun does not need another wrapper edit first
...
- the matched detached `bootstrap44` admin-boost follow-up path is checked in too: `scripts/local/run_qwen_bootstrap44_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_adminoversample_tmux.sh` layers the same extra `site_shopping_admin` warmup coverage onto that strongest expansion-family recipe if the current bootstrap rerun still shows early admin softness
...
- the matched detached `web_mix91` admin-boost follow-up path is checked in too: `scripts/local/run_qwen_web_mix91_curriculum_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_adminoversample_tmux.sh` now carries that same strongest mixed-family recipe forward while also boosting `site_shopping_admin` warmup coverage if the bootstrap rerun still indicates that slice needs more supervision
