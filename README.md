...
- the dedicated detached rerun wrapper is now checked in too: `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearfix_tmux.sh` replays the exact `v20` out-dir/session recipe directly instead of requiring manual environment overrides
...
- `scripts/validate_combined_family_stage_scorecard.py` now validates those combined scorecards explicitly, including source coverage, overlap-task bookkeeping, and merged stage consistency for artifacts like `outputs/web_mix91_current_stack_scorecard.json`
