...
- current quantized GitLab-coverage rerun to watch: `outputs/qwen_bootstrap41_curriculum_v17_qlora_gitlabcoverage_cleandemos/`, which keeps the same checked-in `bootstrap41` split, `bnb_4bit` step-weight recipe, GitLab budget, and GitLab coverage bump as `v16` but now also includes the clean-demo fix from the start
