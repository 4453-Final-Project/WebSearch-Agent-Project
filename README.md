...
- the dedicated detached rerun wrappers are now checked in too: `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearfix_tmux.sh` replays the exact `v20` recipe directly, and `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproductfix_tmux.sh` replays the exact `v21` out-dir/session recipe directly instead of requiring manual environment overrides
...
- the matched detached `bootstrap44` year-fix promotion path is checked in too: `scripts/local/run_qwen_bootstrap44_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearfix_tmux.sh` carries that same strongest recipe forward with the year-scoped admin-bestseller fix pinned into the expansion-family launch surface
...
- the matched detached `web_mix91` year-fix promotion path is checked in too: `scripts/local/run_qwen_web_mix91_curriculum_qlora_gitlabcoverage_warmupoversample_yearfix_tmux.sh` now carries that strongest mixed-family recipe forward with the same year-scoped admin-bestseller fix baked into the launch path
...
- the matched mixed-family year-fix promotion path is ready too: `scripts/local/run_qwen_web_mix88_curriculum_qlora_gitlabcoverage_warmupoversample_yearfix_tmux.sh` now carries that same strongest `web_mix88` recipe forward with the year-scoped admin-bestseller fix baked into the launch surface
