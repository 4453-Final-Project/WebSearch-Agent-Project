...
- current quantized GitLab-budget comparison rerun to watch: `outputs/qwen_bootstrap41_curriculum_v15_qlora_gitlabsteps_cleandemos/`, which keeps the same checked-in `bootstrap41` split, `bnb_4bit` step-weight recipe, and `site_gitlab=8` budget but now also uses the cleaned direct-answer warmup demos from the start
