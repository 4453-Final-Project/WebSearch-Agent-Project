...
- current quantized GitLab-coverage comparison rerun to watch: `outputs/qwen_bootstrap41_curriculum_v18_qlora_gitlabcoverage_cleandemos_warmupoversample/`, which keeps the same checked-in `bootstrap41` split, `bnb_4bit` step-weight recipe, GitLab budget, GitLab coverage bump, and clean-demo fix as `v17` but now also oversamples the GitLab slice during supervised warmup
