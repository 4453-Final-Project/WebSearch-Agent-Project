...
- bootstrap current-stack validator: `scripts/local/validate_bootstrap41_current_stack_summary.sh` and `.ps1` now sanity-check that saved `bootstrap41` current-stack family summary too, so the refreshed scorecard is checked directly from a repo-owned wrapper
...
- bootstrap blocker-audit validator: `scripts/validate_webarena_blocker_audit.py` and `scripts/local/validate_bootstrap41_blocker_audit.sh` / `.ps1` now sanity-check that saved blocker report for task coverage plus mismatch bookkeeping
...
- expanded-scorecard local validator: `scripts/local/validate_bootstrap44_current_stack_scorecard.sh` and `.ps1` now replay that validation directly on the saved `bootstrap44` scorecard, so the expanded artifact has the same repo-owned validation path as the base `bootstrap41` current-stack summary
- expanded-scorecard local audit wrapper: `scripts/local/audit_bootstrap44_current_stack_scorecard.sh` and `.ps1` now replay the saved `bootstrap44` audit too, so the inherited-vs-added lift story stays reproducible from a repo-owned wrapper instead of only through the build path
...
- focused validation for that fix is now saved too: `outputs/eval_task_0_yearproductfix_v2/` and `outputs/eval_task_3_yearproductfix_v1/` both score `1.0`, so the narrowed dashboard-row precedence plus alphabetical tie-breaking recovers the two yearly product tasks directly on the current head before the full-family rerun reaches baseline
- keep the dedicated detached replay paths for those reruns checked in too: `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearfix_tmux.sh` should continue to launch the exact `v20` recipe, `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproductfix_tmux.sh` should continue to launch the exact `v21` recipe, and `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_tmux.sh` should continue to launch the exact `v22` recipe without manual environment overrides
- keep the queued admin-boost follow-up path checked in too: `scripts/local/run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_adminoversample_tmux.sh` should stay aligned with the strongest current bootstrap recipe while adding extra `site_shopping_admin` warmup coverage, so a `v23` rerun can start immediately if `v22` still shows early admin weakness
- keep the matched year-product-tiebreak promotion paths checked in for the next families too: `scripts/local/run_qwen_bootstrap44_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_tmux.sh` and `scripts/local/run_qwen_web_mix91_curriculum_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_tmux.sh` should stay aligned with the strongest current bootstrap recipe before we scale up
- keep the matched admin-boost follow-up paths checked in for the next families too: `scripts/local/run_qwen_bootstrap44_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_adminoversample_tmux.sh` and `scripts/local/run_qwen_web_mix91_curriculum_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_adminoversample_tmux.sh` should stay aligned with the strongest current bootstrap recipe while adding extra `site_shopping_admin` warmup coverage if we need the same follow-up on expansion families
...
- keep the order-only subset validators checked in too: `scripts/local/validate_shopping_order_current_stack_scorecard.sh` / `.ps1` and `scripts/local/validate_shopping_order_full_current_stack_scorecard.sh` / `.ps1` now need to stay aligned with those saved subset scorecards so the order-heavy artifacts have the same repo-owned validation path as the bootstrap and mixed-family scorecards
- keep the order-only subset audit wrappers checked in too: `scripts/local/audit_shopping_order_current_stack_scorecard.sh` / `.ps1` and `scripts/local/audit_shopping_order_full_current_stack_scorecard.sh` / `.ps1` now need to stay aligned with those saved subset scorecards so the order-heavy artifacts keep the same refresh-plus-validate-plus-audit coverage as the bootstrap expansion artifacts
...
- mixed-family local validators: `scripts/local/validate_web_mix88_current_stack_scorecard.sh` / `.ps1` and `scripts/local/validate_web_mix91_current_stack_scorecard.sh` / `.ps1` now replay that combined-scorecard validation directly on the saved mixed-family artifacts, so both `web_mix88` and `web_mix91` have the same repo-owned validation path as the bootstrap scorecards
- mixed-family local audit wrappers: `scripts/local/audit_web_mix88_current_stack_scorecard.sh` / `.ps1` and `scripts/local/audit_web_mix91_current_stack_scorecard.sh` / `.ps1` now replay the saved combined-family audits too, so the mixed-family scorecards keep the same refresh-plus-validate-plus-audit coverage as the expansion artifacts
...
- the matched mixed-family year-product-tiebreak promotion path is ready too: `scripts/local/run_qwen_web_mix88_curriculum_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_tmux.sh` should keep that strongest `web_mix88` recipe aligned with the current narrowed yearly-product dashboard precedence and deterministic dashboard tie-break before we scale up
- the matched mixed-family admin-boost follow-up path is ready too: `scripts/local/run_qwen_web_mix88_curriculum_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_adminoversample_tmux.sh` should keep that strongest `web_mix88` recipe aligned with the same extra `site_shopping_admin` warmup coverage if we need the follow-up beyond bootstrap
