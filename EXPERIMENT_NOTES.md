...
- both quantized bootstrap comparison runs are now live in parallel: `v6_qlora` is the straight weighted baseline, while `v7_qlora_rewardtune` adds the stronger reward shaping on the same checked-in split so we can compare quantized training signal without changing the task family
