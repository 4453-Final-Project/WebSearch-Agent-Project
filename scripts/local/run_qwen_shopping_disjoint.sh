...
OUT_DIR="$WORKSPACE_ROOT/outputs/qwen_shopping_disjoint_v1"
...
python3 scripts/plot_shopping_searchsort_results.py \
  --run-dir "$OUT_DIR" \
  --out-dir "$WORKSPACE_ROOT/outputs/qwen_shopping_report_figures" \
  --run-name qwen_shopping_disjoint_v1 \
  --model-name Qwen/Qwen3.5-2B \
  --title-suffix "Qwen Disjoint Run"
