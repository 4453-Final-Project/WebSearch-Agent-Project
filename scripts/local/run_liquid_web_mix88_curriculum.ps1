param(
    [string]$OutDir = "$(Split-Path $PSScriptRoot -Parent -Parent)\..\outputs\liquid_web_mix88_curriculum_v1"
)

$repoRoot = Split-Path $PSScriptRoot -Parent -Parent
$workspaceRoot = Split-Path $repoRoot -Parent
$manifestPath = Join-Path $repoRoot "scripts\local\web_mix88_curriculum_manifest.json"
...
python scripts/run_family_curriculum.py `
    --family web_mix88 `
    --model-dir-name LiquidAI-LFM2-350M `
