...
$defaultOutDir = Join-Path $workspaceRoot "outputs\qwen_web_mix88_curriculum_v2_qlora_gitlabcoverage"
$runOutDir = if ($env:RUN_OUT_DIR) { $env:RUN_OUT_DIR } else { $defaultOutDir }
$sessionName = if ($env:TMUX_SESSION_NAME) { $env:TMUX_SESSION_NAME } else { "qwen_web_mix88_v2_qlora_gitlabcoverage" }
...
$command = @(
    "cd '$scriptDirWsl'"
    "bash ./run_qwen_web_mix88_curriculum_qlora_gitlabcoverage_tmux.sh"
) -join " && "
