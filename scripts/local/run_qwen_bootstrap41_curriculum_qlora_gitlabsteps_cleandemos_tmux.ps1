...
$defaultOutDir = Join-Path $workspaceRoot "outputs\qwen_bootstrap41_curriculum_v15_qlora_gitlabsteps_cleandemos"
$runOutDir = if ($env:RUN_OUT_DIR) { $env:RUN_OUT_DIR } else { $defaultOutDir }
$sessionName = if ($env:TMUX_SESSION_NAME) { $env:TMUX_SESSION_NAME } else { "qwen_bootstrap41_v15_qlora_gitlabsteps_cleandemos" }
...
$command = @(
    "cd '$scriptDirWsl'"
    "bash ./run_qwen_bootstrap41_curriculum_qlora_gitlabsteps_cleandemos_tmux.sh"
) -join " && "
