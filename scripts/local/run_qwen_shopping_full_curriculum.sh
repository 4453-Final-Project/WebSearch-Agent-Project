...
OUT_DIR="$WORKSPACE_ROOT/outputs/qwen_shopping_full_curriculum_v1"
LOCAL_MANIFEST_PATH="$REPO_ROOT/scripts/local/shopping_full_curriculum_manifest.json"
PREFLIGHT_PATH="$WORKSPACE_ROOT/outputs/qwen_shopping_full_curriculum_preflight_v1/preflight.json"
...
if [[ -f "$LOCAL_MANIFEST_PATH" ]]; then
  ARGS+=(--split-manifest "$LOCAL_MANIFEST_PATH")
elif [[ -f "$PREFLIGHT_PATH" ]]; then
  ARGS+=(--split-manifest "$PREFLIGHT_PATH")
fi
