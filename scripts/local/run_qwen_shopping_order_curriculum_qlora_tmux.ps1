...
if (-not $env:RUN_OUT_DIR) {
    $env:RUN_OUT_DIR = Join-Path $workspaceRoot "outputs/qwen_shopping_order_curriculum_v1_qlora"
}

if (-not $env:TMUX_SESSION_NAME) {
    $env:TMUX_SESSION_NAME = "qwen_shopping_order_v1_qlora"
}
...
$bashScript = (Resolve-Path (Join-Path $scriptDir "run_qwen_shopping_order_curriculum_qlora_tmux.sh")).Path
