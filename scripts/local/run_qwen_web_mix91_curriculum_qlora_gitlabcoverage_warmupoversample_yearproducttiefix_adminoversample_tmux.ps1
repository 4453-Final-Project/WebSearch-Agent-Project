...
$defaultOutDir = Join-Path $workspaceRoot "outputs\qwen_web_mix91_curriculum_v5_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_adminoversample"
$env:RUN_OUT_DIR = if ($env:RUN_OUT_DIR) { $env:RUN_OUT_DIR } else { $defaultOutDir }
$env:TMUX_SESSION_NAME = if ($env:TMUX_SESSION_NAME) { $env:TMUX_SESSION_NAME } else { "qwen_web_mix91_v5_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_adminoversample" }
...
$command = @(
    "cd '$scriptDirWsl'"
    "bash ./run_qwen_web_mix91_curriculum_qlora_gitlabcoverage_warmupoversample_yearproducttiefix_adminoversample_tmux.sh"
) -join " && "
