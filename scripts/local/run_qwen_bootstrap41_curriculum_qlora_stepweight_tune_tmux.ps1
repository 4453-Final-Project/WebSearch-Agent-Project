...
$defaultOutDir = Join-Path $workspaceRoot "outputs\qwen_bootstrap41_curriculum_v8_qlora_stepweight_tune"
$runOutDir = if ($env:RUN_OUT_DIR) { $env:RUN_OUT_DIR } else { $defaultOutDir }
$sessionName = if ($env:TMUX_SESSION_NAME) { $env:TMUX_SESSION_NAME } else { "qwen_bootstrap41_v8_qlora_stepweight_tune" }
...
$command = @(
    "cd '$scriptDirWsl'"
    "bash ./run_qwen_bootstrap41_curriculum_qlora_stepweight_tune_tmux.sh"
) -join " && "
