...
$defaultOutDir = Join-Path $workspaceRoot "outputs\qwen_bootstrap41_curriculum_v23_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_adminoversample"
$env:RUN_OUT_DIR = if ($env:RUN_OUT_DIR) { $env:RUN_OUT_DIR } else { $defaultOutDir }
$env:TMUX_SESSION_NAME = if ($env:TMUX_SESSION_NAME) { $env:TMUX_SESSION_NAME } else { "qwen_bootstrap41_v23_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_adminoversample" }
...
$command = @(
    "cd '$scriptDirWsl'"
    "bash ./run_qwen_bootstrap41_curriculum_qlora_gitlabcoverage_cleandemos_warmupoversample_yearproducttiefix_adminoversample_tmux.sh"
) -join " && "
