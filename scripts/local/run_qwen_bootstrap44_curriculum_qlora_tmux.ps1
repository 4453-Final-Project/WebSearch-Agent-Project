...
$defaultOutDir = Join-Path $workspaceRoot "outputs\qwen_bootstrap44_curriculum_v1_qlora"
$runOutDir = if ($env:RUN_OUT_DIR) { $env:RUN_OUT_DIR } else { $defaultOutDir }
$sessionName = if ($env:TMUX_SESSION_NAME) { $env:TMUX_SESSION_NAME } else { "qwen_bootstrap44_v1_qlora" }
$env:RUN_OUT_DIR = $runOutDir
$env:TMUX_SESSION_NAME = $sessionName

$command = @(
    "cd '$scriptDirWsl'"
    "bash ./run_qwen_bootstrap44_curriculum_qlora_tmux.sh"
) -join " && "
